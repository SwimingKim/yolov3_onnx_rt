...
class DarkNetParser(object):
    ...
    def _parse_params(self, param_line):
        ...
        param_type, param_value_raw = param_line.split(str('='))
        param_value = None
        if param_type == 'layers':
            layer_indexes = list()
            for index in param_value_raw.split(str(',')):
...
class GraphBuilderONNX(object):
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        if self.input_tensor is None:
            print("layer_type : ", layer_type, layer_name)
            if layer_type == 'net':
...
def main():
    ...
    width, height = parse_cfg_wh(cfg_file_path)
    ...
    output_file_path = weights_file_path.replace(".weights", '.onnx')
    onnx.save(yolov3_model_def, output_file_path)
    print("onnx_path", output_file_path)
